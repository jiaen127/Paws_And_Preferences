...
class PawsAndPreferences {
    constructor() {
        ...
        this.isAnimating = false; // Prevent rapid clicks causing duplicates
        this.startPosition = { x: 0, y: 0 };
        this.currentPosition = { x: 0, y: 0 };
        this.currentCard = null;
        // Constants
        this.SWIPE_THRESHOLD = 100; // pixels
        this.ROTATION_MULTIPLIER = 0.1; // degrees per pixel
     ...
     */
    handleDragStart(e) {
        if (this.currentIndex >= this.cats.length || this.isAnimating)
            return;
        const target = e.target;
        const card = target.closest('.cat-card');
        // Only allow dragging the top card
        if (!card || card !== this.cardStack.firstElementChild)
            return;
    ...
    handleDragMove(e) {
        ...
        const dislikeOverlay = this.currentCard.querySelector('.dislike-overlay');
    ...
    swipeCard(liked) {
        ...
        const topCard = this.cardStack.firstElementChild;
        if (!topCard) {
    ...
    storeUndoState(cat) {
        ...
        const timer = this.undoContainer.querySelector('.undo-timer');
        if (timer) {
     ...
     */
    handleUndo() {
        if (!this.undoState || this.currentIndex === 0)
            return;
